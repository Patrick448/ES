...
void GRNEDOHelpers::initializeGRN5Context(appContext* ctx, int mode, int granularity)
{
    ...
    ctx->MIN_K = 0.01;        // Menor valor que K pode assumir
    ctx->MAX_K = 1;          // Maior valor que K pode assumir
    ctx->MIN_N = 0.01;          // Menor valor que N pode assumir
    ctx->MAX_N = 100;         // Maior valor que N pode assumir
    ctx->MIN_TAU = 0.01;      // Menor valor que TAU pode assumir
    ctx->MAX_TAU = 100;        // Maior valor que TAU pode assumir
